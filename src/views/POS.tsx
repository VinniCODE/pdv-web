...
export function POS() {
  ...
  const handleSelectProduct = (sku: string) => {
    ...
    if (success) {
      ...
      setError('Produto sem estoque suficiente!');
    }
  };
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      if (suggestions.length > 0) {
        handleSelectProduct(suggestions[0].sku);
  ...
  // Retorna um ícone diferente dependendo do nome do produto para ficar visual
  const getProductIcon = (name: string) => {
    ...
    return <Smartphone size={24} className="text-white" />; // Padrão para iPhone/Mac
  };
    ...
    <div className="flex gap-6 h-full">
      <div className="flex-1 flex flex-col space-y-4">
        <header>
          ...
          <p className="text-gray-400 mt-1">Operador: Vinícius | Turno: Manhã</p>
        </header>

        {/* Barra de Busca e Dropdown Preditivo */}
        <div className="relative z-50">
          ...
          {/* O Dropdown Altamente Visual (Descolado do fundo) */}
          {suggestions.length > 0 && (
            <div className="absolute top-[110%] left-0 w-full bg-[#2C2C2E] border border-[#48484A] rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.7)] overflow-hidden max-h-[400px] overflow-y-auto">
              <div className="bg-[#3A3A3C] px-4 py-2 text-xs text-gray-400 font-bold uppercase tracking-wider">
              ...
              {suggestions.map((product) => (
                <div 
                  ...
                  className={`p-4 flex items-center justify-between border-b border-[#3A3A3C] last:border-0 cursor-pointer transition-all ${
                    product.stock_current > 0 ? 'hover:bg-[#3A3A3C]' : 'opacity-40 cursor-not-allowed'
                  }`}
                >
                  <div className="flex items-center gap-4">
                    {/* "Foto" do Produto (Ícone visual) */}
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className="bg-black/50 text-gray-300 text-xs px-2 py-0.5 rounded font-mono border border-[#48484A]">
                          {product.sku}
                        </span>
                  ...
                  <div className="flex items-center gap-8 text-right pr-2">
                    <div className="flex flex-col items-end">
                      ...
                      <span className={`flex items-center gap-1 font-bold text-lg bg-black/30 px-3 py-1 rounded-lg border border-[#48484A] ${product.stock_current > 0 ? 'text-green-400' : 'text-red-400'}`}>
                        <Package size={16} />
                    ...
                    <div className="w-32">
                      ...
                      <p className="text-aurora-primary font-bold text-xl tracking-tight">
        ...
        <div className="bg-aurora-card rounded-2xl border border-aurora-border flex-1 overflow-hidden flex flex-col shadow-sm relative z-10">
          <div className="bg-black/20 p-4 border-b border-aurora-border flex justify-between items-center">
            ...
            <span className="bg-aurora-primary text-white px-3 py-1 rounded-full text-sm font-bold">
              {cart.length} itens
            </span>
          </div>
            ...
            {cart.map((item, index) => (
              <div key={item.id} className="flex items-center justify-between bg-black/40 p-4 rounded-xl border border-aurora-border group transition-all hover:border-aurora-primary/50">
                <div className="flex items-center gap-4">
                  ...
                  <div className="w-10 h-10 bg-[#2C2C2E] rounded-lg flex items-center justify-center border border-aurora-border">
                ...
                <div className="flex items-center gap-6">
                  ...
                  <button onClick={() => removeFromCart(item.id)} className="text-gray-500 hover:text-red-400 transition-colors p-2 rounded-lg hover:bg-red-400/10">
                    <Trash2 size={20} />
                  </button>
                </div>
              </div>
      ...
      <div className="w-96 flex flex-col space-y-4 relative z-10">
        ...
        <div className="bg-aurora-card p-6 rounded-2xl border border-aurora-border shadow-sm">
          ...
          <div className="space-y-4 text-lg">
            ...
            <div className="flex justify-between text-3xl font-bold text-white pt-4 border-t border-aurora-border mt-4">
              <span>Total</span>
              <span>R$ {subtotal.toLocaleString('pt-BR', {minimumFractionDigits: 2})}</span>
            </div>
          </div>
        ...
        <div className="bg-aurora-card p-6 rounded-2xl border border-aurora-border flex-1 shadow-sm flex flex-col">
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            <button className="flex flex-col items-center justify-center p-4 rounded-xl border-2 border-aurora-primary bg-aurora-primary/10 text-aurora-primary transition-all">
              <QrCode size={28} className="mb-2" />
              <span className="font-bold mt-1">PIX</span>
            </button>
            <button className="flex flex-col items-center justify-center p-4 rounded-xl border border-aurora-border bg-black/40 text-gray-400 hover:border-gray-300 hover:text-white transition-all">
              <CreditCard size={28} className="mb-2" />
              <span className="font-bold mt-1">Cartão</span>
            </button>
          </div>
          <button 
            onClick={checkout}
